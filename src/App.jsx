...
async function sendEmail(to,subject,html){
  ...
  try{
    await fetch('https://api.resend.com/emails',{
      ...
      body:JSON.stringify({from:'Just Koul <contact@justkoul.com>',to:[to],subject,html})
    });
...
function LoginModal({onLogin,onClose,data}){
  ...
  const doVerifyForgot=async()=>{
    ...
    if(Date.now()>verfExpiry){setVerfErr("Code expiré. Veuillez recommencer.");return;}
    setVerfBusy(true);setVerfErr("");
    const{error}=await supabase.auth.resetPasswordForEmail(fEmail.trim(),{redirectTo:"https://www.justkoul.com/reset-password"});
    setVerfBusy(false);
    if(error){setVerfErr(error.message);return;}
